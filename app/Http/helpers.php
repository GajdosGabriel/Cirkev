...
//Aktívne menú
	...
	function googleMap()
	{
		//  Google map and component  https://github.com/bradcornford/Googlmapper
		$users = \App\User::where('Lat', '<>', 48.78003500000000); //  because this Lat is default
		\Mapper::location('Banská Bystrica')->map(['zoom' => 8, 'marker' => false]);

		if (request('mapUsers')) {
			$users->where(request('mapUsers'), '=', 1);
		}
		...
		foreach ($users as $user) {
			if ($user->lat !== null) {
				$options = ['marker' => true, 'title' => 'title for map', 'eventMouseOver' => 'showProfile( ' . json_encode($user) . ' )', 'eventMouseOut' => 'hideProfile()'];
				\Mapper::marker($user->lat, $user->lng, $options);
			}
		}
	}
...
{
	return link_to( URL::previous(), 'späť' );
}
...
//Odstránenie zložky a obrázku pri zmazaní
